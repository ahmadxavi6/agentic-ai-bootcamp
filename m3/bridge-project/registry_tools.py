...
def gleif_lookup(legal_name: str) -> str:
    ...
    try:
        ...
        # 404 is the register answering "nothing under that name". Any other
        # non-200 is the call not completing - a different thing entirely.
    ...
    # 2. JSON:API - the company is several layers down, under
    #    data[] -> attributes -> entity / registration.
    ...
    for rec in records:
        ...
        found_name = ((entity.get("legalName") or {}).get("name") or "?").strip()
        lei = attrs.get("lei") or rec.get("id") or "?"
        country = (
            (entity.get("legalAddress") or {}).get("country")
            or entity.get("jurisdiction")
            or "?"
        )
        is_exact = _flat(found_name) == asked
        exact += is_exact

        candidates.append(
            f"  - {found_name}"
            f"{'   <- named EXACTLY what you asked for' if is_exact else ''}\n"
            f"      LEI: {lei}\n"
            f"      country: {country}\n"
            # 3. Two different fields. ACTIVE/INACTIVE is whether the company
            #    still legally exists; ISSUED/LAPSED/RETIRED is whether its LEI
            #    record is still being maintained. LAPSED = the entity stopped
            #    paying/revalidating its LEI, so the data is no longer certified
            #    current. It is NOT evidence the company is dead.
            f"      entity status: {entity.get('status') or 'UNKNOWN'}\n"
            f"      registration status: {registration.get('status') or 'UNKNOWN'}"
        )

    header = (
        f"GLEIF register: {len(candidates)} candidate(s) under "
        f"'{name}' - {exact} exactly matching that name."
    )
    if exact != 1:
        header += (
            "  WARNING: the register cannot tell you which of these is your"
            " supplier." if exact == 0
            else "  WARNING: more than one record carries this exact name."
        )
